...
# Plotting generation time distribution
...
sns.barplot(
    ax=axes[1, 0], data=agg_df, x="num_rollouts", y="tflops_seq_gen", hue="model_size"
)
axes[1, 0].set_title(
...
def plot_rewards_heatmap(num_rollouts_range, model_sizes, max_tflops_budget):
    ...
    plt.title(
        f"Heatmap of Predicted Reward (Missing cells indicate total TFLOPS > Budget ({max_tflops_budget} TFLOPS)"
    )
...
reward_df_melted = reward_df.reset_index().melt(
    id_vars="num_rollouts", var_name="model_size", value_name="predicted_test_reward"
)
